...
大语言模型为高级机器人任务规划提供了有前景的交互接口，但在多无人机协同场景中，系统化评估需要面向任务层的环境，包括受限 API、基于角色的信息访问、局部观测、隐藏验证逻辑和闭环执行。MultiUAV-Plat 围绕这一需求，提供 RESTful 多无人机仿真平台、可复现测评基准，以及用于规划、执行、验证与重规划的 Agent4Drone 工作流。

### 特色

- 面向任务级 UAV 控制、感知、会话管理和验证的 RESTful APIs。
- 基于角色的访问控制与 agent-facing observations，避免 agent 直接访问特权仿真状态。
- 隐藏任务验证器，用于可复现的闭环测评。
- 支持 2D 和 3D 可视化，覆盖任务全局查看、场景编辑、UAV 轨迹、高度、覆盖区域、目标、障碍物、小地图上下文、跟随/漫游相机和截图导出。
- GUI controller 支持会话创建、编辑、导入/导出和监控。
- MultiUAV-Agent Workflow 以 Agent4Drone 作为参考实现。
...
- [1. 启动系统](#1-启动系统)
  - [1.1 无人机仿真系统](#11-无人机仿真系统)
  - [1.2 无人机控制系统](#12-无人机控制系统)
- [2. 编辑整体场景](#2-编辑整体场景)
  - [2.1 创建场景](#21-创建场景)
  - [2.2 打开场景](#22-打开场景)
  - [2.3 导出场景](#23-导出场景)
...
- [5. 任务管理](#5-任务管理)
  - [5.1 添加任务](#51-添加任务)
  - [5.2 标记任务完成情况](#52-标记任务完成情况)
  - [5.3 任务模板](#53-任务模板)
- [6. AI 智能体自动检查](#6-ai-智能体自动检查)
  - [6.1 启动 AI 智能体服务（agent4drone）](#61-启动-ai-智能体服务agent4drone)
  - [6.2 进入自动检查界面](#62-进入自动检查界面)
  - [6.3 导入会话](#63-导入会话)
  - [6.4 开始检查](#64-开始检查)
- [7. 3D 可视化查看器](#7-3d-可视化查看器)
  - [7.1 环境要求与界面概览](#71-环境要求与界面概览)
  - [7.2 启动 3D 查看器](#72-启动-3d-查看器)
  - [7.3 界面说明](#73-界面说明)
  - [7.4 基本操作](#74-基本操作)
  - [7.5 键盘快捷键](#75-键盘快捷键)
  - [7.6 与后端联动的场景可视化](#76-与后端联动的场景可视化)
  - [7.7 漫游模式](#77-漫游模式)
