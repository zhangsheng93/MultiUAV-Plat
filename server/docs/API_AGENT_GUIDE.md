...
`X-Agent-ID` is a non-secret client identity label used for AGENT request-history filtering. Autonomous clients should send a stable value for every request. If omitted, AGENT requests are attributed to `default_agent`.
...
- inspect its own current-session request history.
...
Command responses use semantic `status` values. `success` means the requested command completed fully. For `move_along_path`, `partial_success` means `allow_partial_move=true` let the drone reach at least one waypoint but stop before the final requested waypoint because an obstacle or insufficient battery blocked the remaining path. `error` means no allowed movement was executed for the failed command. Only successful or partially successful `move_along_path` responses include `successful_points_count`, `successful_points`, `unsuccessful_points_count`, and `unsuccessful_points`; point lists contain normalized `(x, y, z)` triples.
...
AGENT request-history access is intentionally narrow: `GET /sessions/current/request-history` returns only AGENT-authenticated records with the same normalized `X-Agent-ID`. It does not expose SYSTEM, ADMIN, USER, other-agent, or arbitrary-session request history.

An AGENT cannot clear request history. `DELETE /sessions/current/request-history` and `DELETE /sessions/{session_id}/request-history` are reserved for SYSTEM/ADMIN tooling.
